...
from apps.factura.models import (
    FactureLine, Client, QuotaFacture
)
...
class DeleteReportFactureView(View):
    def post(self, request, *args, **kwargs):
        id_report = kwargs['pk']
        QuotaFacture.objects.filter(facture__id=id_report).delete()
        FactureLine.objects.filter(id=id_report).delete()
